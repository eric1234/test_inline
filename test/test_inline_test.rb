...
class TestInlineTest < Test::Unit::TestCase
  ...
  def test_registration_with_parent
    ...
    assert Test::Inline.test_case_for(__FILE__).instance_methods.collect {|m| m.to_s}.include?("test_xyz_test_inline_test_rb_24")
    assert_equal 'baz', Test::Inline.test_case_for(__FILE__).new(:test_xyz_test_inline_test_rb_24).test_xyz_test_inline_test_rb_24
    assert Test::Inline.test_case_for(__FILE__).instance_methods.collect {|m| m.to_s}.include?("test_abc_test_inline_test_rb_25")
    assert_equal 'dog', Test::Inline.test_case_for(__FILE__).new(:test_abc_test_inline_test_rb_25).test_abc_test_inline_test_rb_25
  ...
  def test_auto_build
    ...
    Test('def') {'bar'}
    $0 = old_script
    assert Test::Inline.test_case_for(__FILE__).instance_methods.collect {|m| m.to_s}.include?("test_def_test_inline_test_rb_47")
    assert_equal 'bar', Test::Inline.test_case_for(__FILE__).new(:test_def_test_inline_test_rb_47).test_def_test_inline_test_rb_47
  end
